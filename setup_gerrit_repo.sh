...
gerrit_template_repo_name=${GERRIT_TEMPLATE_REPO_NAME:-'interflo-template-projects'}
...
echo ${encoded_gerrit_password}

# Http url with gerrit user credentials
gerrit_authorized_url=http://"${gerrit_username}:${encoded_gerrit_password}@${gerrit_domain}/a"

echo ${gerrit_authorized_url}

# Install checks plugin
plugin_id="checks"
plugin_source='"https://gerrit-ci.gerritforge.com/job/plugin-checks-bazel-stable-3.3/lastSuccessfulBuild/artifact/bazel-bin/plugins/checks/checks.jar"'
...
# Create new interflo repo template
./setup_interflo_template.sh  "$gerrit_authorized_url" "$gerrit_username" "$gerrit_user_email" "$gerrit_template_repo_name"
